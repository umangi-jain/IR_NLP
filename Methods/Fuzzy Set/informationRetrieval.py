...
class InformationRetrieval():
	...
	def buildIndex(self, docs,titles, docIDs):
		...
		self.cooccur = np.zeros([len(self.vocab), len(self.vocab)])
		...
		for term in self.vocab:
			self.vocab_map[term] = kk
			kk +=1

		print(np.shape(self.cooccur))
		self.L = np.zeros([len(self.vocab), len(self.docIDs)])
		print("shape of L matrix: ", np.shape(self.L))
		for tt in range(len(self.vocab)):
			if(tt%200 == 0):
				print("term ", tt)
			for dd in range(len(docs)):
				aa = 1
				for sentence in docs[dd]:
					for word in sentence:
						aa = aa*(1-self.cooccur[tt][self.vocab_map[word]])
				try:
					self.L[tt, dd] = 1- aa
				except:
					print(tt, dd, aa)
		print("Constructed L matrix")
	...
	def rank(self, queries):
		...
		for i in range(len(queries)):
			...
			for docId in self.docIDs:
				...
				for sentence in query:
					for word in sentence:
						if word in self.vocab_map.keys():
							sim = sim*self.L[self.vocab_map[word],(docId-1)]

				cosine_similarity_scores.append((docId,sim))
				if (i == 0 and docId < 3):
					print("attention ", sim, docId)
